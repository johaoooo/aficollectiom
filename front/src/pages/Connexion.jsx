...
import { Envelope, Lock, Eye, EyeSlash, ArrowRight, GoogleLogo, FacebookLogo, CheckCircle, WarningCircle, SignIn } from '@phosphor-icons/react'
...
export default function Connexion() {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (!formData.email || !formData.password) { setError('Veuillez remplir tous les champs'); return } setSubmitting(true); setTimeout(() => { setSubmitting(false); setSubmitted(true); setTimeout(() => setSubmitted(false), 2000) }, 800) }
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-sm mx-auto px-4 py-8">
        <motion.div variants={fadeUp} initial="hidden" animate="show" className="text-center mb-6">
          <motion.div custom={0} variants={fadeUp} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full mb-3 bg-[#008753]/10 dark:bg-[#008753]/15"><SignIn size={10} weight="duotone" className="text-[#008753]" /><span className="text-[9px] font-semibold tracking-widest uppercase text-[#008753]">Connexion</span></motion.div>
          <motion.h2 custom={1} variants={fadeUp} className="text-gray-900 dark:text-white mb-1" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(1.8rem, 5vw, 2.5rem)', fontWeight: 600 }}>Bonjour</motion.h2>
          <motion.p custom={2} variants={fadeUp} className="text-[10px] text-gray-500 dark:text-gray-400">Connectez-vous à votre compte</motion.p>
        </motion.div>
        ...
        <motion.div custom={3} variants={fadeUp} className="relative rounded-2xl overflow-hidden mb-4 bg-white dark:bg-gray-800/50 shadow-lg dark:shadow-none border border-gray-200 dark:border-white/10">
          ...
          <div className="p-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-2.5">
              ...
              <div className="flex items-center justify-between text-[9px]"><label className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400"><input type="checkbox" className="w-2.5 h-2.5 rounded border-gray-300 dark:border-gray-600" /> Se souvenir</label><a href="#" className="text-[#008753] hover:underline">Mot de passe oublié ?</a></div>
              <button type="submit" disabled={submitting || submitted} className="w-full py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all bg-gradient-to-r from-[#008753] to-[#006b42] text-white">{submitting ? <span className="text-xs">Connexion...</span> : submitted ? <><CheckCircle size={12} weight="fill" /><span className="text-xs">Connecté !</span></> : <><ArrowRight size={12} /><span className="text-xs">Se connecter</span></>}</button>
            </form>
          </div>
        </motion.div>
        ...
        <p className="text-center text-[10px] text-gray-500 dark:text-gray-400 mt-5">Pas encore de compte ? <Link to="/inscription" className="text-[#008753] font-semibold hover:underline">S'inscrire</Link></p>
      </div>
    </main>
  )
}
